...
def onSelect(event):
	...
	for entry in fields:
		selected_patient.__setattr__(entry,entries[entry].get())
	patient = listbox.curselection()[0]
	selected_patient = patients_list[patient]
	for entry in fields:
		entries[entry].delete(0, END)
		entries[entry].insert(0, selected_patient.__getattribute__(entry))
	calculate_health()
...
def add_patient():
	...
	listbox.insert(END, patient)

def del_patient():
	selected_patient = listbox.curselection()[0]
	listbox.delete(selected_patient)
	patients_list.remove(patients_list[selected_patient])
...
Label(frame, text="	Patients").pack()

listbox = Listbox(frame)
listbox.pack(side=LEFT)
listbox.bind('<<ListboxSelect>>', onSelect)
listbox.insert(0, patient1)
...
Button(frame, text='Add', command= add_patient).pack()
Button(frame, text='Delete', command= del_patient).pack()
